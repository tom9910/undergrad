...
%token INT AND OR NOT VOID WHILE DO IF ELSE READ WRITE MYBEGIN THEN MYRETURN TRUE FALSE BOOLEAN END FOR
%token LE GE EE NE
...
%type <node> expression simpleExpr addExpr factor term readstmt var iterationstmt assignmentstmt exprstmt selectionstmt call returnstmt for
...
for : FOR '(' assignmentstmt ';' expression ';' assignmentstmt ')' statement{
        $$ = CreateASTNode(FORLOOP);
        $$->s1 = $3;
        $$->s2 = CreateASTNode(FORLOOP1);
        $$->s2->s1 = $5;
        $$->s2->s2 = CreateASTNode(FORLOOP2);
        $$->s2->s2->s1 = $7;
        $$->s2->s2->s2 = $9;
